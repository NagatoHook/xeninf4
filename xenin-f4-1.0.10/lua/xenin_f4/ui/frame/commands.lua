				...
				if (v.input) then
					...
					local translate = function(str)
						...
						if (v.dontTranslate) then
							...
							return F4Menu:GetPhrase(str)
						end
					end

					F4Menu:Modal(translate(v.name), 
						translate(input.desc or v.name),
						translate(isfunction(input.str) and input.str(ply) or input.str or ""),
						translate(isfunction(input.placeholder) and input.placeholder(ply) or input.placeholder or ""),
						input.numeric,
